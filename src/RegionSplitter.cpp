...
bool RegionSplitter::polarSort(const Eigen::Vector2d & a, const Eigen::Vector2d & b)
{
    double angle_a = std::atan2(a[1], a[0]);
    double angle_b = std::atan2(b[1], b[0]);
                            ...
                            const cv::Mat & depth_img)
{
    ...
    double angle_threshold = M_PI / 6.0; // 30 degrees
    ...
    for (int i = 0; i < (int) centers_lc.size(); i++)
    {
        ...
        std::vector<Eigen::Vector2d> superpixel_projection = superpixel_projections[i];

        // Sort points by bearing from center
        std::vector<Eigen::Vector2d> superpixel_projection_sorted = superpixel_projection;
        ...
        {
            Eigen::Vector2d prev_point = superpixel_projection_sorted[j - 1];
            Eigen::Vector2d current_point = superpixel_projection_sorted[j];
            ...
            double angle_prev = std::atan2(prev_point[1], prev_point[0]);
            double angle_current = std::atan2(current_point[1], current_point[0]);

            double angle_diff = angle_current - angle_prev;
            if (angle_diff < 0)
            {
                angle_diff += 2.0 * M_PI;
            }
