...
Eigen::Vector3f ConvexHullifier::projectPointOntoPlane(const Eigen::Vector3f & regionPt)
{
    Eigen::Vector3f planeCenter = Eigen::Vector3f(0, 0, 0); // center is the origin
    Eigen::Vector3f planeNormal = Eigen::Vector3f(0, 0, 1);
    Eigen::Vector3f diff = regionPt - planeCenter; 
    float dist = diff.dot(planeNormal);
    Eigen::Vector3f projPt = regionPt - dist * planeNormal;

    return projPt;
}

void ConvexHullifier::run(std::vector<std::vector<float>> & centers,
                            ...
                            const cv::Mat & depth_img)
{
    ...
    superpixel_projections = std::vector<std::vector<Eigen::Vector2f>>(centers.size());
    superpixel_convex_hulls = std::vector<std::vector<Eigen::Vector2f>>(centers.size());
    egocan_to_region_rotations = std::vector<Eigen::Matrix3f>(centers.size());
    ...
    Eigen::Vector3f arbitraryVec(1, 0, 0);

    Eigen::Vector3f e0, e1;

    Eigen::Matrix3f egocanToRegionRotMat;
    Eigen::Matrix4f egocanToRegionTransform;
    ...
    Eigen::Vector4f egocanPtHomog;
    Eigen::Vector4f regionPtHomog;
    Eigen::Vector3f regionPt;
    Eigen::Vector3f projPt;
    ...
    for (int i = 0; i < (int) centers.size(); i++)
    {
        ...
        {
            arbitraryVec = Eigen::Vector3f(0, 1, 0);
        }
        ...
        // Eigen::Quaternionf regionQuat(egocanToRegionRotMat);
        ...
        // Eigen::VectorXf region_pose_world_frame = transformHelperPoseStamped(center, regionQuat, egocanFrameToWorldFrame);

        // // get rotation matrix
        // Eigen::Matrix3f rotMat = calculateRotationMatrix(region_pose_world_frame[3], region_pose_world_frame[4], region_pose_world_frame[5]);

        // Eigen::Matrix4f worldToRegionTransform = Eigen::Matrix4f::Identity();
        // worldToRegionTransform.block<3, 3>(0, 0) = rotMat;
        // worldToRegionTransform.block<3, 1>(0, 3) = region_pose_world_frame.head(3); 

        // Eigen::Matrix4f egocanToWorldTransform = Eigen::Matrix4f::Identity();
        // // quaternion to rotation matrix
        // Eigen::Quaternionf egocanToWorldQuat(egocanFrameToWorldFrame.transform.rotation.w, 
        ...
        // Eigen::Matrix3f egocanToWorldRotMat = egocanToWorldQuat.toRotationMatrix();
        // egocanToWorldTransform.block<3, 3>(0, 0) = egocanToWorldRotMat;
        // Eigen::Vector3f egocanToWorldTrans(egocanFrameToWorldFrame.transform.translation.x,
        ...
        // Eigen::Matrix4f egocanToRegionTransform = egocanToWorldTransform * worldToRegionTransform;
        ...
        // {
            ...
            // if (superpixel_convex_hulls[i][j] == Eigen::Vector2f(0.0, 0.0)) // if a hull point is too close to origin
            // {
            ...
            //         Eigen::Vector4f egocanPtHomog(egocanPt.val[0], egocanPt.val[1], egocanPt.val[2], 1.0);
            //         Eigen::Vector4f regionPtHomog = egocanToRegionTransform * egocanPtHomog;

            //         Eigen::Vector3f regionPt = regionPtHomog.head(3);

            //         RCLCPP_WARN_STREAM(logger_, "               (region frame): " << regionPt.transpose());

            //         // project points onto plane
            //         Eigen::Vector3f projPt = projectPointOntoPlane(regionPt);
...
void ConvexHullifier::convexHull(const std::vector<Eigen::Vector2f> & superpixel_projections, 
                                    std::vector<Eigen::Vector2f> & convex_hull)
{
    grahamScan(superpixel_projections, convex_hull);
}

bool ConvexHullifier::polarSort(const Eigen::Vector2f & a, const Eigen::Vector2f & b, const Eigen::Vector2f & lowest)
{
    if (a == lowest)
    {
        return true;
    ...
    float angle_a = std::atan2(a[1] - lowest[1], a[0] - lowest[0]);
    float angle_b = std::atan2(b[1] - lowest[1], b[0] - lowest[0]);
...
bool ConvexHullifier::ccw(const Eigen::Vector2f & a, const Eigen::Vector2f & b, const Eigen::Vector2f & c)
{
    Eigen::Vector2f a_to_b = b - a;
    Eigen::Vector2f a_to_c = c - a;
    return (a_to_b[0] * a_to_c[1] - a_to_b[1] * a_to_c[0]) > 0;
}

void ConvexHullifier::grahamScan(const std::vector<Eigen::Vector2f> & superpixel_projections, 
                                    std::vector<Eigen::Vector2f> & convex_hull)
{
    ...
    std::vector<Eigen::Vector2f> sorted = superpixel_projections;
